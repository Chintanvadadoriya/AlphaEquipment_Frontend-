...
const Rented = () => {
  ...
  const getCategory = async (pageNo = page) => {
    setLoader(true);
    setPage(pageNo);
    const result = await categoryDetail(`?pagination=${pagination}&pageNo=${pageNo}&limit=${limit}&sellType=${sellType}`);
    setLoader(false);
  ...
  const handleRedirectSubcategory = async (categoryId) => {
    Router.push({
      pathname: router.RENTED.SUBCATEGORY,
                          ...
                          {categoryData?.map((item, index) => {
                            return (
                              <div className="category-box-detail" key={index}>
                                <Link href={{ pathname: router.RENTED.SUBCATEGORY, query: { categoryId: item?.id } }}>
                                  <img src="/assets/category-img.png" alt="category-img" />
                                  <p>
                                    {item?.name} <span>({item?.subCategoryCount})</span>
                                  </p>
                                </Link>
